...
smc:
	$(GORUN) build/ci.go install ./cmd/smc
	@echo "Done building."
...
devtools:
	...
	env GOBIN= go install github.com/jteeuwen/go-bindata/go-bindata@latest
...
smc-linux-386:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/386 -v ./cmd/smc
	@echo "Linux 386 cross compilation done:"
	@ls -ld $(GOBIN)/smc-linux-* | grep 386

smc-linux-amd64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/amd64 -v ./cmd/smc
...
smc-linux-arm-5:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/arm-5 -v ./cmd/smc
	@echo "Linux ARMv5 cross compilation done:"
	@ls -ld $(GOBIN)/smc-linux-* | grep arm-5

smc-linux-arm-6:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/arm-6 -v ./cmd/smc
	@echo "Linux ARMv6 cross compilation done:"
	@ls -ld $(GOBIN)/smc-linux-* | grep arm-6

smc-linux-arm-7:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/arm-7 -v ./cmd/smc
	@echo "Linux ARMv7 cross compilation done:"
	@ls -ld $(GOBIN)/smc-linux-* | grep arm-7

smc-linux-arm64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/arm64 -v ./cmd/smc
	@echo "Linux ARM64 cross compilation done:"
	@ls -ld $(GOBIN)/smc-linux-* | grep arm64

smc-linux-mips:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/mips --ldflags '-extldflags "-static"' -v ./cmd/smc
	@echo "Linux MIPS cross compilation done:"
	@ls -ld $(GOBIN)/smc-linux-* | grep mips

smc-linux-mipsle:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/mipsle --ldflags '-extldflags "-static"' -v ./cmd/smc
	@echo "Linux MIPSle cross compilation done:"
	@ls -ld $(GOBIN)/smc-linux-* | grep mipsle

smc-linux-mips64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/mips64 --ldflags '-extldflags "-static"' -v ./cmd/smc
	@echo "Linux MIPS64 cross compilation done:"
	@ls -ld $(GOBIN)/smc-linux-* | grep mips64

smc-linux-mips64le:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=linux/mips64le --ldflags '-extldflags "-static"' -v ./cmd/smc
...
smc-darwin-386:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=darwin/386 -v ./cmd/smc
	@echo "Darwin 386 cross compilation done:"
	@ls -ld $(GOBIN)/smc-darwin-* | grep 386

smc-darwin-amd64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=darwin/amd64 -v ./cmd/smc
...
smc-windows-386:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=windows/386 -v ./cmd/smc
	@echo "Windows 386 cross compilation done:"
	@ls -ld $(GOBIN)/smc-windows-* | grep 386

smc-windows-amd64:
	$(GORUN) build/ci.go xgo -- --go=$(GO) --targets=windows/amd64 -v ./cmd/smc
