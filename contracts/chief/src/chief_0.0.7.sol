...
//import "github.com/MeshBoxFoundation/meshbox/contracts/chief/src/chief_abs_s0.5.sol"; // for remix
...
contract TribeChief_0_0_7 is Chief {
    ...
    constructor(address[] memory genesisSigners, uint _epoch, uint _signerLimit, uint _volunteerLimit) public {
        ...
        if (len > 0) {
            ...
            address g1 = address(uint160(371457687117486736155821182390123011782146942855));
    ...
    function repeatTi(uint[] memory tiList, uint ti) private pure returns (bool) {
        if (tiList.length > 0) {
    ...
    */
    ...
    function fixRule1() private {
        ...
        for (uint i = 0; i < _signerList.length; i++) {
            if (uint160(_signerList[i]) == uint160(0)) {
                deleteSigner(i);
                i--;
            }
        }
